...
void VROUniformBinder::setForMaterial(const void *value, const VROGeometry *geometry, const VROMaterial *material) {
    if (_uniform->getLocation() == -1) {
        return;
    }
    _bindingBlock(_uniform, geometry, material);
}
