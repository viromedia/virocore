...
4. Zip up the AAR files back into an archive. IMPORTANT: do NOT zip the core-X.Y.Z.aar directory; instead, go *inside* that directory and zip up the files themselves! This is to ensure that the resulting AAR doesn't extract out into a directory (the files should be "flat"). Zip the files by selecting all the ones you want in Finder and Right Click -> Compress (Using `tar` from the command line didn't work for me - Andy).
