...
lowp vec4 blinn_lighting_diffuse_fixed(VROSurface surface,
...
lowp vec4 blinn_lighting_diffuse_texture(VROSurface surface,
                                         highp vec3 camera_position,
                                         lowp vec4 diffuse_texture_color,
                                         sampler2D specular_texture) {

    lowp vec3 light_ambient_color = ambient_light_color.xyz * surface.diffuse_color.xyz * diffuse_texture_color.xyz;
    
    lowp vec4 material_diffuse_color  = surface.diffuse_color * diffuse_texture_color * surface.diffuse_intensity;
    lowp vec4 material_specular_color = texture(specular_texture, surface.specular_texcoord);
    highp vec3 surface_to_camera = normalize(camera_position - surface.position);
